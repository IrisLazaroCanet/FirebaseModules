...
class MessagesAdapter(table: RecyclerView): RecyclerView.Adapter<MessageViewHolder>() {
    ...
    init {
        FB.dataBase.OnTableChange<DataBaseMessage>(DataBaseMessage().GetTable(), onChange = { updatedMessages ->
    ...
    fun AddMessage(text: String?, imageUrl: String?) {
        val dataBaseMessage = DataBaseMessage(null, FB.auth.GetUser()?.id, text, imageUrl)

        FB.dataBase.save(dataBaseMessage,
            onSuccess = {
                //TODO EXTRA CONTROL
            },
            onFailure = {
